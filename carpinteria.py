...
def conectar_gsheets():
    # 1. Intentar cargar desde los secretos de Streamlit (Nube)
    if "gcp_service_account" in st.secrets:
        # Construimos el diccionario forzando el reemplazo correcto de saltos de línea
    ...
    # 2. Intentar cargar desde variables de entorno (Otras plataformas)
    elif "GCP_SERVICE_ACCOUNT" in os.environ:
        service_account_info = json.loads(os.environ["GCP_SERVICE_ACCOUNT"])
        creds = Credentials.from_service_account_info(
            service_account_info, scopes=SCOPES
        )
    # 3. Intentar cargar desde archivo local (Para cuando programes en tu PC)
    else:
...
if not st.session_state.autenticado:
    ...
    with st.expander("🚨 ¿Olvidaste tu contraseña? Solicita acceso aquí"):
        st.write(
            "Ingresa tus datos. El administrador revisará y aprobará tu cambio en el panel."
        )
    ...
    if opcion == "Inventario":
        ...
        try:
            ...
            if cliente:
                ...
                st.error("No se pudieron cargar las credenciales de Google Sheets. Revisa tus secretos.")
            ...
            with tab4:
                ...
                if st.button("Restablecer Contraseña"):
                    ...
                    if usuario_reset.lower() in df_users["Usuario"].values:
                        ...
                        df_users.to_csv(ARCHIVO_¡Listo! No te preocupes, ya me encargué de arreglar toda la lógica de conexión para que los saltos de línea de la contraseña no rompan nada. 

Sigue estos **dos únicos pasos** y ya coronamos:

### Paso 1: Actualiza tu archivo en VS Code
Copia todo este código que te dejo aquí abajo y pégalo en tu archivo **`carpinteria.py`** (borra todo lo que tengas ahí y pega esto). Guárdalo y súbelo a GitHub como ya sabes (`git add .`, `git commit -m "Fix credenciales"`, `git push origin main`).
...
def conectar_gsheets():
    if "gcp_service_account" in st.secrets:
        # Aquí arreglamos el problema de los saltos de línea automáticamente
